...
class BasicAgentSA(object):
    """
    BasicAgentSA implements an agent that navigates the scene.
    This agent respects traffic lights and other vehicles, but ignores stop signs.
    It has several functions available to specify the route that the agent must follow,
    as well as to change its parameters in case a different driving mode is desired.
    """

    def __init__(self, vehicle, target_speed=20, opt_dict={}, map_inst=None, grp_inst=None, mode = None):
        """
    ...
    def set_user_throttle(self, throttle):
        """
        Set the user's throttle input.
        :param throttle: Throttle value from the user input (range from 0.0 to 1.0).
        """
        self._user_throttle = throttle

    def set_user_brake(self, brake):
        """
        Set the user's brake input.
        :param brake: Brake value from the user input (range from 0.0 to 1.0).
        """
        self._user_brake = brake

    def set_user_steer(self, steer):
        """
        Set the user's steer input.
        :param steer: Steering value from the user input (range from -1.0 to 1.0).
        """
        self._user_steer = steer
    ...
    def run_step(self):
        ...
        if self._mode == "Throttle":
            if hasattr(self, '_user_throttle'):
                control.throttle = self._user_throttle  # Override throttle with user input
    
        if self._mode == "Brake":
            if hasattr(self, '_user_brake'):
                control.brake = self._user_brake  # Override brake with user input

        if self._mode == "Steer":
            if hasattr(self, '_user_steer'):
                control.steer = self._user_steer # Override steer with user input
